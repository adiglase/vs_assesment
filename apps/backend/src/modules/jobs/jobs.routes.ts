...
import { assignEditor, assignReporter, completeJob, markReviewed, markTranscribed, type WorkflowErrorCode } from './jobs.workflow'
...
jobsRouter.post('/jobs/:id/complete', (req, res) => {
  const parsedParams = jobIdParamsSchema.safeParse(req.params)
  ...
  const db = req.app.locals.db as Database.Database
  const result = completeJob(db, parsedParams.data.id)
